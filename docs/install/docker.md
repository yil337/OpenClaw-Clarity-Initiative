...
- **Protect the gateway from JSON typos:** Before editing `~/.openclaw/openclaw.json`, run a lint + backup step (`python3 -m json.tool <file>` or the config-guard workflow) so that malformed configs never hit disk.
