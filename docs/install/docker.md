...
- **Protect the gateway from JSON typos:** Before editing `~/.openclaw/openclaw.json`, run a lint + backup step (`python3 -m json.tool <file>` / `jq` + temp file) so malformed configs never hit disk；若需自动化，可参考后续 Tooling PR 中的实验性 config-guard add-on。
