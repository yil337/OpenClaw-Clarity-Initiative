...
| **Control UI 授权黑屏** | Browser keeps showing blank panel even after token entry; stale localStorage or Service Worker cache. | In Chrome: DevTools → Application → Clear storage (incl. SW). Then run `localStorage.setItem('gatewayToken', 'PASTE_TOKEN')`, refresh, and approve device again. Safari: `Develop → Empty Caches` before reinjecting token。 | 2‑3 min |
